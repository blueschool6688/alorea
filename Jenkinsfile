pipeline {
    ...
    stages {
        ...
        stage('Deploy to Server') {
            steps {
                script {
                    sshagent(credentials: [SERVER_SSH_DEPLOY]) {
                        sh """
                    ...
                    sh """
                        curl -s -H "Content-Type: application/json" -X POST -d '{
                            "embeds": [{
                                ...
                                "description": "Dự án **${IMAGE_NAME}** đã được cập nhật thành công lên server! 🚀",
                                "color": 3066993,
                                "fields": [
                                    { "name": "🔢 Build Number",    "value": "#${env.BUILD_NUMBER}", "inline": true },
                                    { "name": "🏷️ Docker Tag",      "value": "${IMAGE_TAG}",         "inline": true },
                    ...
                    sh """
                        curl -s -H "Content-Type: application/json" -X POST -d '{
                            "embeds": [{
                                ...
                                "description": "Quá trình CI/CD dự án **${IMAGE_NAME}** gặp lỗi! Vui lòng kiểm tra console log Jenkins.",
                                "color": 15158332,
                                "fields": [
                                    { "name": "🔢 Build Number",    "value": "#${env.BUILD_NUMBER}", "inline": true },
                                    { "name": "🏷️ Docker Tag",      "value": "${IMAGE_TAG}",         "inline": true },
